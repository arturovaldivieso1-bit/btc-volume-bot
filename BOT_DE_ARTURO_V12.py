...
# Sistema de bias y scoring (MEJORADO)
EMA_SHORT = 50
EMA_LONG = 200
SCORE_UMBRAL_ACCION = 5   # lo dejamos igual porque funciona
SCORE_MAX_TEORICO = 30    # se recalculará automáticamente

PESOS = {
    "zona_spot": 1,
    "zona_oi_base": 2,          # se suma peso_por_oi además de esto
    "toques_altos": 1,
    "cercania": 1,
...
# Peso adicional por magnitud de OI
...
# Umbrales de volumen (estudio) - sin cambios
VOL1_MED = 400
VOL1_P75 = 692
VOL1_P90 = 1134
PROB1_MED = 68
...
historial_eventos = deque(maxlen=1000)
ultimos_eventos = deque(maxlen=5)  # para detección de patrones
...
def detectar_zonas_oi(df_oi, df_spot):
    ...
    for i in range(1, len(df_oi)):
        ...
        nuevos_incrementos.append(max(0, oi_actual - oi_anterior))
    ...
    for i in range(OI_LOOKBACK, len(df_oi)):
        ...
        for j in range(i - OI_LOOKBACK + 1, i + 1):
            ...
            suma += max(0, oi_actual - oi_anterior)
        if suma > umbral_dinamico:
...
def calcular_peso_zona(zona_spot, zona_oi, precio_actual):
    ...
    if zona_spot:
        ...
        if distancia(zona_spot, precio_actual) < 0.005:
            peso += PESOS["cercania"]
    if zona_oi:
        peso += PESOS["zona_oi_base"] + peso_por_oi(zona_oi["oi_total"])
...
def calcular_score_evento(evento_tipo, direccion_evento, bias, peso_zona, validacion_spot=False):
    ...
    elif (bias == "ALCISTA" and direccion_evento == "ALCISTA") or (bias == "BAJISTA" and direccion_evento == "BAJISTA"):
        score += PESOS["bias_favorable_tendencia"]
    return score

def normalizar_score(score):
    # SCORE_MAX_TEORICO se recalcula al inicio
    return round(score / SCORE_MAX_TEORICO * 10, 1)
...
def enviar_liquidez_detectada(mejor_zona_oi_arriba, mejor_zona_oi_abajo, mejor_zona_spot_arriba, mejor_zona_spot_abajo, precio, hora, bias):
    ...
    def enviar_zona(zona, tipo, es_oi, zonas_spot_referencia=None):
        ...
        if es_oi:
            ...
            linea_extra = f"OI acumulado: +{oi_valor:.1f}M USD {zona.get('confianza', '')}"
        else:
            icono = "💰"
            fuente = "SPOT"
...
def radar_impulse(df_entry, precio_actual, zonas_arriba, zonas_abajo, bias):
    global last_impulse_time, last_event_time, historial_eventos, ultimos_eventos
    ...
    ultimos_eventos.append(f"impulso_{direccion}")
...
def radar_sweep(df_entry, mejor_zona_arriba, mejor_zona_abajo, precio_actual, zonas_arriba, zonas_abajo, bias):
    global sweep_pendiente, last_event_time, historial_eventos, ultimos_eventos
    ...
    if sweep_pendiente:
        ...
        if 'oi_total' in zona:
            for z in (zonas_arriba + zonas_abajo):
                if 'toques' in z and abs(zona['centro'] - z['centro']) / zona['centro'] < SPOT_FUTUROS_TOLERANCIA:
                    validacion_spot = True
                    break
        score = calcular_score_evento("sweep", direccion_rev, bias, peso_zona, validacion_spot)
        score_norm = normalizar_score(score)
        if score_norm < SCORE_UMBRAL_ACCION:
            sweep_pendiente = None
            return
        ...
        ultimos_eventos.append(f"sweep_{direccion_rev}")
        last_event_time = ahora
        sweep_pendiente = None
        return
    if len(df_entry) < 2:
...
def radar_breakout(df_entry, mejor_zona_arriba, mejor_zona_abajo, precio_actual, zonas_arriba, zonas_abajo, bias):
    global zona_consumida, last_event_time, alerted_liquidity, historial_eventos, ultimos_eventos
    ...
    def hubo_retest(zona, direccion):
        ...
        for i in range(2, BREAKOUT_RETEST_CANDLES + 2):
            ...
            if direccion == "ALCISTA" and vela_ant["close"] > zona["min"] and vela_ant["close"] < zona["max"]:
                return True
            if direccion == "BAJISTA" and vela_ant["close"] > zona["min"] and vela_ant["close"] < zona["max"]:
                return True
        return False
        ...
        if key not in alerted_liquidity and close > mejor_zona_arriba.get("max", mejor_zona_arriba["centro"]*1.01) * (1 + BREAKOUT_MARGIN):
            if hubo_retest(mejor_zona_arriba, "ALCISTA"):
                ...
                if score_norm >= SCORE_UMBRAL_ACCION:
                    ...
                    ultimos_eventos.append("breakout_ALCISTA")
                    last_event_time = ahora
                    return
        ...
        if key not in alerted_liquidity and close < mejor_zona_abajo.get("min", mejor_zona_abajo["centro"]*0.99) * (1 - BREAKOUT_MARGIN):
            if hubo_retest(mejor_zona_abajo, "BAJISTA"):
                ...
                if score_norm >= SCORE_UMBRAL_ACCION:
                    ...
                    ultimos_eventos.append("breakout_BAJISTA")
                    last_event_time = ahora
                    return
...
def detectar_patrones():
    if len(ultimos_eventos) < 3:
        return
    ultimos_3 = list(ultimos_eventos)[-3:]
    if ultimos_3 == ["sweep_BAJISTA", "sweep_BAJISTA", "sweep_BAJISTA"]:
        enviar("⚠️ PATRÓN DETECTADO: 3 sweeps bajistas consecutivos (posible acumulación bajista)")
    elif ultimos_3 == ["sweep_ALCISTA", "sweep_ALCISTA", "sweep_ALCISTA"]:
        enviar("⚠️ PATRÓN DETECTADO: 3 sweeps alcistas consecutivos (posible acumulación alcista)")
    # Podríamos añadir más patrones (ej. impulso + sweep, etc.)
...
def evaluar():
    ...
    radar_proximidad(mejor_oi_arriba if mejor_oi_arriba else mejor_spot_arriba,
                     mejor_oi_abajo if mejor_oi_abajo else mejor_spot_abajo,
                     precio, hora_str, bias)

    radar_impulse(df_entry, precio, zonas_arriba, zonas_abajo, bias)

    radar_sweep(df_entry, mejor_oi_arriba if mejor_oi_arriba else mejor_spot_arriba,
                mejor_oi_abajo if mejor_oi_abajo else mejor_spot_abajo,
                precio, zonas_arriba, zonas_abajo, bias)

    radar_breakout(df_entry, mejor_oi_arriba if mejor_oi_arriba else mejor_spot_arriba,
                   mejor_oi_abajo if mejor_oi_abajo else mejor_spot_abajo,
                   precio, zonas_arriba, zonas_abajo, bias)
...
if __name__ == "__main__":
    print("🚀 Iniciando BOT V12.1 (con mejoras: score normalizado, peso OI escalado, bias lateral, detección de patrones)...")
    # Calcular SCORE_MAX_TEORICO dinámicamente
    SCORE_MAX_TEORICO = sum(PESOS.values()) + 4 + 4  # +4 por peso_oi máximo, +4 por cercanía máxima (dos cercanías)
    # Aproximadamente 30, pero lo dejamos así.
    ...
    if os.path.exists(HISTORIAL_FILE):
        try:
            with open(HISTORIAL_FILE, "r") as f:
                data = json.load(f)
                historial_eventos.extend(data[-1000:])
        except:
            pass
